...
RANGE_CHART_SYSTEM_PROMPT = "\n".join([
    ...
    "      \"range_top\": \"Bed 9 (youngest occurrence, Yinkeng Fm)\" (string, the YOUNG/upper limit),",
    ...
    "- READ SPECIES NAMES CAREFULLY. Names are small italic Latin binomials, often densely packed and rotated at an angle. OCR can produce slight misreads. When a genus name is ambiguous, prefer a plausible known radiolarian genus and keep the epithet. Do not merge two names, do not split one name, and do not invent names. P-2 FIX: Preserve open nomenclature qualifiers exactly as printed: sp., cf., aff., ?, spp. are meaningful taxonomic distinctions — do not strip or normalize them.",
